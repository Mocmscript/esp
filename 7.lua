		...
		if Find then
			...
			if MyChar and GetChar then
				...
				if Find2 and Find3 then
					...
					if Find4 then
						Find.Frame.Health.Text = "血量: " .. string.format("%.0f", Find4.Health)
					else
...
function InitMain()
	...
	Title.Text = "做这个脚本我他妈快疯了xd 作者huh 脚本版本7.0.8.5"
	...
	RemoveToBlacklist.Text = "从黑名单中删除"
	...
	RemoveToWhitelist.Text = "从好友列表中删除"
...
提问:
1) 如何使用自瞄?
A：如果你是电脑你需要先绑定一个启用自瞄的按键

2) ESP/上色 无法运行在此游戏怎么办
A: 通常是服务器更换了玩家id从而导致脚本无法读取玩家id

3) 当玩家在墙后面时，我是如何检测到的？
A：将摄像机光线投射到另一个玩家。

4) 我的子弹在使用自瞄的时候无法命中敌人?
A: 子弹扩散吧
日志
xd
	...
	if Bullshit.DebugInfo then
		...
		if Bullshit.ClosestEnemy ~= nil then
			...
			if MyTor and GetTor then
				...
				DebugMenu["PlayerPosition"].Text = "Position: (X: " .. string.format("%.3f", Pos.X) .. " Y: " .. string.format("%.3f", Pos.Y) .. " Z: " .. string.format("%.3f", Pos.Z) .. ") 距离: " .. string.format("%.0f", Dist) .. " Studs"
